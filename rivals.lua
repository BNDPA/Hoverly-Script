--[[
    Hub Name: Hoverly Script | Rivals
    UI Library: WindUI
--]]

local WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/Footages/WindUI/main/init.lua"))()
...
Tabs.Aimbot:Toggle({
    Title = "Enable Aimbot",
    Description = "Locks camera view onto target bone",
    Default = false,
...
Tabs.Aimbot:Slider({
    Title = "Smoothness",
    Description = "Higher value = slower, smoother snap",
    Min = 1,
    Max = 20,
    Default = 5,
    Callback = function(val)
...
Tabs.Aimbot:Slider({
    Title = "FOV Radius",
    Description = "Field of view circle restriction",
    Min = 30,
    Max = 400,
    Default = 120,
    Callback = function(val)
...
Tabs.Aimbot:Toggle({
    ...
    Description = "Redirects shots toward target automatically",
    Default = false,
    Callback = function(state)
...
Tabs.Visuals:Toggle({
    ...
    Description = "Unlocks all weapon skins, wraps, and cosmetics client-side",
    Default = true,
    Callback = function(state)
...
-- [Skins Unlocker Background Logic Execution]
task.spawn(function()
    ...
    if hookmetamethod then
        ...
        if eqrem then
            ...
            oncall = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                if not getgenv().HoverlyConfig.ESP.SkinsUnlocker then return oncall(self, ...) end
    ...
    if citem and citem._CreateViewModel then
        ...
        citem._CreateViewModel = function(self, vmref)
            ...
            if getgenv().HoverlyConfig.ESP.SkinsUnlocker and wplr == lp and equip[wname] and equip[wname].Skin and vmref then
    ...
    if vmmod then
        ...
        if cvm.GetWrap then
            ...
            cvm.GetWrap = function(self)
                if not getgenv().HoverlyConfig.ESP.SkinsUnlocker then return ogw(self) end
        ...
        cvm.new = function(rdata, cliitm)
            ...
            if getgenv().HoverlyConfig.ESP.SkinsUnlocker and wplr == lp and equip[wname] then
            ...
            if getgenv().HoverlyConfig.ESP.SkinsUnlocker and wplr == lp and equip[wname] and equip[wname].Wrap and res._UpdateWrap then
                res:_UpdateWrap()
    ...
    ilib.GetViewModelImageFromWeaponData = function(self, wdata, hires)
        if not wdata or not getgenv().HoverlyConfig.ESP.SkinsUnlocker then return ogvi(self, wdata, hires) end
    ...
    if cent and cent._PlayFinisher then
        ...
        cent._PlayFinisher = function(self, fname, ...)
            if not getgenv().HoverlyConfig.ESP.SkinsUnlocker then return ofin(self, fname, ...) end
            local ewep = getewep()
...
Tabs.World:Toggle({
    Title = "FullBright (Remove Darkness)",
    Default = false,
    Callback = function(state)
...
Tabs.Settings:Button({
    Title = "Unload Hub",
    Description = "Removes UI and stops active loops",
    Callback = function()
        WindUI:Destroy()
    end
})
...
-- Notify user that script has successfully loaded
WindUI:Notify({
    Title = "Hoverly Script | Rivals",
    Content = "Successfully loaded UI framework and Skins Unlocker!",
    Duration = 5
})
